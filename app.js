...
function playGame() {
  if (setting.start) {
    ...
    if (keys.ArrowLeft && setting.x > 0) {
      setting.x -= setting.speed; // если кнопка влево зажата то двигаемся по оси X влево
    }
    if (keys.ArrowRight && setting.x < gameArea.offsetWidth - car.offsetWidth) {
      //если кнопка влево зажата то двигаемся по оси X вправо
    ...
    car.style.left = setting.x + "px"; // число с добавлением px
    car.style.top = setting.y + "px"; // проверка условия

    requestAnimationFrame(playGame); // рекурсия - снова выполяется ф-ция playgame по кругу пока start.true
  }
}
